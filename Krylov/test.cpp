...
vector<double> GMRESnoPre(vector<vector<double> > &A, vector<double> &b, int m=10)
{
  int n = b.size();
  vector<double> x0(n);	// Initial guess (zero vector)
	vector<double> xm = copyVec(x0);
...
int main()
{
  ...
  vector<double> b = {0, 0, 0, 0, 1, 5, 1, 0, 0, 0};
	...
	// cout << "\nGMRES:\n" ;
	vector<double> x = GMRESnoPre(A, b);
	printVec(x);

  // vector<vector<double> > P = preconditioner(A, "LR");
	// printVec2D(P);

  // vector<double> y = GMRESPre(A, b);
  // // cout << endl;
  // printVec(y);
