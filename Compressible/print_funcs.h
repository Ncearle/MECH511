...
void printVec2D(vector<vector<double> > vec)
{
	for (int j = 0; j < vec.size(); j++)
	{
		...
		{
			cout << fixed << setprecision(p) << setw(p+4) << vec[j][i];
		}
		cout << endl;
	}
	cout << endl;
...
void printVec3D(vector<vector<vector<double> > > vec, int k)
{
	...
	{
		for (int i = 0; i < vec[k][j].size(); i++)
		{
			cout << fixed << setprecision(p) << setw(p+5) << vec[k][j][i];
		}
		cout << endl;
	}
	cout << endl;
...
void vec3D2File(const string fName0, const string fName1, const string fName2, vector<vector<vector<double> > > &vec) // Prints 3D vector to a file
{
	...
	{
		for (int i = 1; i < vec[0][j].size()-1; i++)
		{
			outfile0 << vec[0][j][i] << " ";		// Separated by a space for .dat files
		}
		outfile0 << endl;
	}
	outfile0 << endl;
	...
	{
		for (int i = 1; i < vec[1][j].size()-1; i++)
		{
			outfile1 << vec[1][j][i] << " ";		// Separated by a space for .dat files
		}
		outfile1 << endl;
	}
	outfile1 << endl;
	...
	for (int j = 1; j < vec[2].size()-1; j++)
	{
		for (int i = 1; i < vec[2][j].size()-1; i++)
		{
			outfile2 << vec[2][j][i] << " ";		// Separated by a space for .dat files
		}
		outfile2 << endl;
	}
	outfile2 << endl;
